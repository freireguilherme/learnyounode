...
filtro(diretorio, extString, (err, list) => { //chama a função definida no modulo
  if (err)
    return console.error('Houve um erro:', err); //error

  list.forEach((file) => { // executa a funcao callback, q retorna err ou data (a lista no caso)
    console.log(file);
  });
});